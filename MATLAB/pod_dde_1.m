function solution = pod_dde_1
'-------------- dde_of_podmems_1 --------------------'
%calls: none
%called by: none 
%
%This solves a PODMEMS delay differential equation, where
...
%---Time delays
    ...
    tauD = 1*tauK; %Delay of FD
    tauM = 1*tauK; %Delay of FM
    tau = [tauK, tauD, tauM];
%---Initial conditions
    history = [0, 0]; %Displacement and velocity prior to t=0
    t_initial = 0; %Start time
    t_final   = 7; %Final time
    tspan = [t_initial t_final]*1e-3; %Simulation time span
...
%Plot
    ...
    clf; 
    grid on; hold on;
    title('PODMEMS DDE displacement vs time'); xlabel('Time [sec]'); ylabel('Displacement [m]');
...
%Structural parameters
    ...
    D = 1.55e-7; %Ns/m
    K = 2; %N/m
    K_nonlin = pi * pi * 170e9 * 2e-6 * 20e-6 / (294.7e-6)^3 / 64;
%Comb params
