...
G = 1e-2;
        ...
        %x0 = 1.99e-6;
        %U_capacitor = 1/2*(-1/2*B*V^2 / (gap-x0));
        ...
        %F(2) =     -1/2*1/2*B*V^2*(gap+x1)^-2   -  (6*-C*(gap+x1)^-7 ); %sum of forces to zero out to find x
        %F(6) =     -1/2*1/2*B*V^2*(gap+x2)^-2   -  (6*-C*(gap+x2)^-7 ); %sum of forces to zero out to find x
        ...
        %J(2,2) = -1/2*2*1/2*B*V^2*(gap+x1)^-3 + 7*(-6*-C*(gap+x1)^-8 ); %dF = slope = delta_F / delta_x
        %J(6,6) = -1/2*2*1/2*B*V^2*(gap+x2)^-3 + 7*(-6*-C*(gap+x2)^-8 ); %dF = slope = delta_F / delta_x
        ...
        %F(3) =   F(6) * params.l;        
        %J(3,3) = -J(6,6) * params.l;
        %F(7) =     F(2) * params.l;
        %J(6,6) = J(2,2) * params.l;
        %F(3) = 0; F(7) = 0;
