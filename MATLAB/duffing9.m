...
%Parameters
    M = 8e-10; %mass 8e-10
    %D = 1.55e-7 %Air
    kappa = 1^2 * 37 * 8.854e-12 * 20e-6 / (2e-6 * (5.28e-6)^3)
    N = 37; e0 = 8.854e-12; 
    V = 1.1
    g = 2e-6; 
%Temperature    
    ...
    if 0
        xmax = 50e-6;
        xdot2 = xmax * w0; 
        w_earth = 2*pi / (24 * 60 * 60); %Radians per sec
        F0 = 2 * M * w_earth * xdot2 %Coriolis amplitude    
        amplitude = linspace(0.00001, 1, 1000000)*8e-6; %Range for displacement amplitude
    end
...
%Figure displacy
    ...
    plot(X1*w0/k/2/pi,Y1,'r'); 
    plot(X3(1:length(X3)-1)*w0/k/2/pi,Y3(1:length(X3)-1),'r');
