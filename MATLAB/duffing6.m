...
%Parameters
    %M = 8e-10; %mass 8e-10
    D = 1*1.55e-7; %damping 1.55e-7
    %K = 2; %stiffness 2
    %kappa = 1 * 4.1e10; %nonlinearity 4.1e10
    kappa = 0 * 2.6e10; %nonlinearity 4.1e10
    N = 70; e0 = 8.854e-12; h = 10e-6; g = 3e-6; 
    V = 7.3;
    F0 = 1*0.5* 2*N*1/2*e0*h/g*V^2; %Force amplitude
    w0 = sqrt(K/M);
    Q = w0*M/D
    xmax = F0/w0/D;
    amplitude = linspace(0.01, 2, 10000)*xmax; %Range for displacement amplitude
%Eqn conversion 
...
%Figure displacy
    ...
    plot(X1*w0/k/2/pi,Y1,'b'); 
    plot(X3*w0/k/2/pi,Y3,'b');
