function [output] = MF_gap2de_h(flag, R, params, q, t, nodes, varargin)
output = [];
switch(flag)
    case 'vars'
...
function [output] = compute_forces(F_dFdx, r, params, q_global, t)
    ...
    %gap conductance
        ...
        V1 = q_local(4)^2; %Applied voltage difference between plates
        V2 = q_local(8)^2; %Applied voltage difference between plates
        gap1 = abs(gap + q_local(2) - q_local(10) - eps);
        gap2 = abs(gap + q_local(6) - q_local(14) - eps);
        %U_capacitor = 1/2*(-1/2*B*V^2 / closure);
        
        %C = (U_capacitor) / closure^-6; %C = (U_spring*0 + U_capacitor) / (gap-x0).^-6;

        delta = .1e-6;
        C1 = -1/2*B*V1^2 / (gap-delta)^2 * (gap-delta)^7;
        C2 = -1/2*B*V2^2 / (gap-delta)^2 * (gap-delta)^7;        
        C1 = -1/2*1/2*B*V1^2*(gap-delta)^-2  /  (6*abs(gap-delta)^-7 );
        C2 = -1/2*1/2*B*V2^2*(gap-delta)^-2  /  (6*abs(gap-delta)^-7 );
        
        F(2) =     -1/2*1/2*B*V1^2*(gap1)^-2   -  (6*-C1*abs(gap1)^-7 ); %sum of forces to zero out to find x
        F(6) =     -1/2*1/2*B*V2^2*(gap2)^-2   -  (6*-C2*abs(gap2)^-7 ); %sum of forces to zero out to find x        
        J(2,2) = -1/2*2*1/2*B*V1^2*abs(gap1)^-3 + 7*(-6*-C1*(gap1)^-8 ); %dF = slope = delta_F / delta_x
        J(6,6) = -1/2*2*1/2*B*V2^2*abs(gap2)^-3 + 7*(-6*-C2*(gap2)^-8 ); %dF = slope = delta_F / delta_x     
        closure = closure*2;
        closure = delta;
        if gap1 <= closure && gap1 >= closure
            G = G/10;
            F(4) = -G * q_local(4); %1e-2;%G * q_local(4); %current = G * voltage
            J(4,4) = -G;
        end
        if gap1 >= closure && gap2 <= closure
            G = G/10;
