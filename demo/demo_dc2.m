...
disp('  1) Netlist of multiple beams subjected to an external moment'); 
...
clc
disp(' ');
disp('1/4) Netlist file: multibeam.net');
disp(' ');
...
disp('deflection:');
disp(' ');
disp('net = cho_load(''multibeam.net'');');
disp(' ');
...
disp('displays the undeflected structure of the netlist');
disp(' ');
disp('To see the shape of the structure better, rotate the');
disp('display by left clicking and dragging on the image');
disp(' ');
...
disp('(for example, the y displacement of the node ''B''), use');
disp(' ');
disp(' dy = cho_dq_view(dq,net,''B'',''y'')');
disp(' ');
disp('where tip is the node name, and y is desired coordinate');
disp('For this example, the computed deflection is');
disp(' ');
dy = cho_dq_view(dq,net,'B','y')
disp(' ');
