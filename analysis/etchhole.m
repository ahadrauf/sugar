...
function etchholecoords(maxseparation,L,W,fid,theta,X,Y)
...
minetchholesize=500; %minimum allowable etch hole dimension
...
for xindex=1:ceil(L/maxseparation)-1;
  ...
  for yindex=1:ceil(W/maxseparation)-1;
      ...
      %patch((x(xindex)+B(1,:))*1e-8,(y(yindex)+B(2,:))*1e-8, color); 
      p=rot2local(0,0,theta)'*[x(xindex)-L/2;y(yindex);0]; %point
      patch((p(1)+X+B(1,:))*1e-8,(p(2)+Y+B(2,:))*1e-8 ,color); %display
 		fprintf(fid, 'B %d %d %d,%d %d,%d; \n',round(minetchholesize),round(minetchholesize),round((p(1)+X+B(1))),round(p(2)+Y+B(2)),round(cos(theta)*100),round(sin(theta)*100))
   end
end