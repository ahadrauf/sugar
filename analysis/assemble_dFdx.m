...
function [dFdx] = assemble_dFdx(net, q, t, is_sp);
...
if (is_sp)
  ...
  dFdx = spalloc(net.dof, net.dof, 6*net.dof);
else
  dFdx = zeros(net.dof);
end
...
for i = 1:length(net.elements)
  ...
  if ~isempty(elt.var_ids)
    ...
    [dFdxlocal] = feval(elt.model, 'dFdx', elt.R, elt.parameter, qlocal, t);

    if (~isempty(dFdxlocal))
      dFdx(jdx,jdx) = dFdx(jdx,jdx) + dFdxlocal(j,j);
    end
