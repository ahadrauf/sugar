...
[a-zA-Z][a-zA-Z0-9_]*    { yylval.id = mempool_strdup(parse_pool, yytext);
                           return ID;      }
{inttok}            { yylval.ival = atoi(yytext);
                      return INT;     }
{floattok}          { yylval.dval = atof(yytext);
                      return FLOAT;   }
{floattok}{numext}  { yylval.dval = atof(yytext) *
...
{
    /* Try the original file name first */
    FILE* fp = fopen(filename, "r");

#ifdef __SUGAR_MEX

    /* If that doesn't work, search the Matlab path */
    if (fp == NULL) {
        mxArray *prhs[1], *plhs[1];

        prhs[0] = mxCreateString(filename);
        mexCallMATLAB(1, plhs, 1, prhs, "which");

        if (mxIsChar(plhs[0])) {
            int namelen = mxGetN(plhs[0])+1;
            filename = (char*) mxCalloc(namelen, sizeof(char));
            mxGetString(plhs[0], filename, namelen);
            fp = fopen(filename, "r");
            mxFree(filename);
        } 
