...
%to call the function use the call cho2cif(net,{'p1'},0)  the net is the netlist that is loaded, 
%p1 is the layer that you  want and 0 is if you want extend beams on or off, in this case it is off.
...
for lyr = 1 : length(layer)%runs through for layers
   ...
   for i=1:length( net.elements ) %check all netlist elements 
      ...
      if isfield(net.elements(i).parameter,layer(lyr)) %must be right layer 
         ...
         R=net.elements(i).R; %theta=net.elements(i).parameter.oz; %get xy-plane orientation 
         theta = acos(R(1,1));
         ...
         for nodesource = 1:length( net.elements(i).node_ids ) %get a node to check 
            for j = 1 : length(net.elements) %check netlist for another 
               if isfield(net.elements(j).parameter,layer{lyr}) %must be right layer 
                  for nodetest=1:length( net.elements(j).node_ids) %get a 2nd node 
                     if (strcmp(net.elements(i).node_ids(nodesource),net.elements(j).node_ids(nodetest))&(i~=j)& isempty(strmatch(net.elements(i).node_ids(nodesource), flagnode,'exact'))) 
                        %has to be same node, but differnet element, and not already checked 
                        flagnode{length(flagnode)+1}=net.elements(i).node_ids(nodesource); %flag it 
         ...
         if length(net.elements(i).node_ids)==2 &(L>=6000 | W>=6000)%etchole
            if (L>4000&W>4000)
         ...
         if length(net.elements(i).node_ids)==1 %anchor
