...
q
   L=param.l;
   o3=zeros(3);o6=zeros(6);
   R6=[R o3;o3 R];R12=[R6 o6;o6 R6];   
   qloc=R12*q; %rotate to local
   
   Q=qloc;
   q(8)=Q(8)-Q(2); %y2 offset
   q(9)=Q(9)-Q(3); %z2 offset
   q(1:3,1)=[0;0;0]; %xyz1 offset
   
   q(6)=asin(q(8)/L); %oz1
   q(12)=q(6); %oz2
   q(5)=asin(q(9)/L); %oy1
   q(11)=q(5); %oy2
   q(7)=-L+L*cos(q(6))*cos(q(5)); %x2
   ...
   q(9)=Q(9); %z2
   q(3)=Q(3); %z1 
   q(7)=q(7)+(Q(7)-Q(1)); %x2 offset
   q(1)=q(1)+(Q(7)-Q(1)); %x1 offset
   
   q=R12'*q;
   displaybeam(real(q), nodes(1).pos, R, param.l, param.w, param.h);
