function [output] = MF_beam_link_grav(flag, R, params, q, t, nodes, varargin);
...
case 'F'
    ...
    m = density * volume;
    g = 9.8;
    Fy = m * g /2; %force on each of 2 nodes in the in-plane y-direction
    Mz = Fy*L/6; %moment about z due to a uniformly distributed load in y direction
    Fz = m * g / 2; %force on each of 2 nodes in the out-of-plane z-direction
    My = Fz*L/6; %moment about z due to a uniformly distributed load in y direction
    output = params.gravity*[0;-Fy;-Fz;  0;0;0;  0;-Fy;-Fz;  0;0;0];
case 'vars'
