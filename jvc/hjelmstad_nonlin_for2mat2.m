...
%      implicit double precision (a-h,o-z)
    ...
    A=zeros(ndm*ndm,1); b=zeros(ndm,1); x=zeros(ndm,1); xo=zeros(ndm,1); D=zeros(3,1); %dimension A(ndm,ndm),b(ndm),x(ndm),xo(ndm),D(3) 
...
%c.... Execute numerical integration of Hessian and residual components
        ...
        [A]=zeros(ndm*ndm,1);    %call zerovec(A,ndm*ndm)
        z = 0;   %z = 0.d0
...
%C.... Add end load terms to the residual and coefficient matrix
        for i=1:nbasis   %do i=1,nbasis
          ...
          A((mm+1)*(ndm-1)+(ndm)) = A((mm+1)*(ndm-1)+(ndm)) - h*Cp;; %A(mm+1,ndm) = A(mm+1,ndm) - h*Cp;
          A((mm+2)*(ndm-1)+(ndm)) = A((mm+2)*(ndm-1)+(ndm)) - h*Cq; %A(mm+2,ndm) = A(mm+2,ndm) - h*Cq;
          A((mm+3)*(ndm-1)+(ndm)) = A((mm+3)*(ndm-1)+(ndm)) - h*Cm; %A(mm+3,ndm) = A(mm+3,ndm) - h*Cm;
        end   %end do
...
%c.... Add arc length constraint terms to  Hessian and residual
        for k=1:ndm %do k=1,ndm
          ...
          A(ndm*(ndm-1)+(k)) = 2*(x(k) - xo(k));  %A(ndm,k) = 2*(x(k) - xo(k));
        end   %end do
...
%c.... Assemble the result into the matrix A 
          for m=1:3  %do m=1,3
            ...
            for n=1:3    %do n=1,3
              nn = 3*(j-1);
              A((mm+m)*(ndm-1)+nn+n) = A((mm+m)*(ndm-1)+nn+n)+ BGB(m,n)*factor_;    %A(mm+m,nn+n) = A(mm+m,nn+n)+ BGB(m,n)*factor_;
            end   %end do
          end     %end do 
...
%c.... Form the integral part of load factor_ part of  matrix A
        ...
        A((mm+1)*(ndm-1)+ndm) = A((mm+1)*(ndm-1)+ndm) - hi*dp*factor_;  %A(mm+1,ndm) = A(mm+1,ndm) - hi*dp*factor_;
        A((mm+2)*(ndm-1)+ndm) = A((mm+2)*(ndm-1)+ndm) - hi*dq*factor_;  %A(mm+2,ndm) = A(mm+2,ndm) - hi*dq*factor_;
        A((mm+3)*(ndm-1)+ndm) = A((mm+3)*(ndm-1)+ndm) - hi*dm*factor_;  %A(mm+3,ndm) = A(mm+3,ndm) - hi*dm*factor_;
