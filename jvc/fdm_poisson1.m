function fdm_poisson1
%   This program solves Poisson equation u_{xx} + u_{yy} = f(x,y),  a < x < b,  c < y < d.
%   with Dirichlet boundary condition along x=b, y=c, y=d, and a Neumann BC at x=a. 
%   Computed result:
%       m = 10, n = 20; 0.0021
...
for j = 1:n-1,
  for i=1:m,
    k = i + (j-1)*m;
    bf(k) = fdm_f_h2_3(x(i),y(j+1));
    A(k,k) = -2/hx1 -2/hy1;
    if i == 1
        A(k,k+1) = 2/hx1;
        bf(k) = bf(k) + 2*fdm_ux_h2_3(y(j+1))/hx;
    else
       if i==m
         A(k,k-1) = 1/hx1;
         bf(k) = bf(k) - fdm_ue_h2_3(x(i+1),y(j+1))/hx1;
       else
	  A(k,k-1) = 1/hx1;
	  A(k,k+1) = 1/hx1;
       end
...
%-- y direction --------------
    ...
    if j == 1
        A(k,k+m) = 1/hy1;
        bf(k) = bf(k) - fdm_ue_h2_3(x(i),c)/hy1;
    else
       if j==n-1
         A(k,k-m) = 1/hy1;
         bf(k) = bf(k) - fdm_ue_h2_3(x(i),d)/hy1;
       else
          A(k,k-m) = 1/hy1;
          A(k,k+m) = 1/hy1;
       end
