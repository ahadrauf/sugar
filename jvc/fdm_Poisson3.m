%fdm_Poisson3
%This program solves Poisson equation u_xx + u_yy = f(x,y) in the domain xmin < x < xmax,  ymin < y < ymax.
%It has Dirichlet boundary conditions along x=xmax, y=ymin, y=ymax, and a Neumann boundary condition at x=min. 
...
function fdm_Poisson3
%By jvclark - summer 2003. 
%From studies of numerical solution of pde's - finite difference methods by Z. Li.

%set boundary limits
xmin=1;    xmax=2;    %xmin < x < xmax
ymin=-1;   ymax=1;    %ymin < y < ymax

%set the mesh resolution
xres=40*5;   %number of elements along x
yres=80*5;   %number of elements along y
...
for j = 1 : yres-1 %for each row
    for i = 1 : xres %for each col
        ...
        elseif i==xres %if x=xmax, at the right x boundary, Dirichlet BC
            A(k,k-1) = 1/hxhx;    %coefficient
            f(k) = f(k) - u_Dirichlet(x(i+1),y(j+1))/hxhx;    %source
        else
            A(k,k-1) = 1/hxhx;  %coefficient 
            A(k,k+1) = 1/hxhx;  %coefficient 
        end %if x boundary
...
function f = u_Dirichlet(x,y) %boundary condition
    %f = x*x + y*y;
    f = exp(-x)*sin(pi*y);
