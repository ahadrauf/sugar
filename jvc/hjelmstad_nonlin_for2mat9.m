...
function [xx,yy,pp]=NonlinearBeam(FY)  
tic
...
test0=0; %questions
test1=0; %book results
test2=0;
test3=0; %force moment
test4=1; %mems Fy
if test0
...
column_length=20e-6;
    ...
    ndm=3*num_basis_functions+1; 
      a_nu = 0.3;
      a_E = 165.0e9;
      a_G = a_E / (2.0 * (1.0 + a_nu));
      a_w = 2.0e-6;
      a_h = 2.0e-6;
      a_area = a_w*a_h;
      a_inertia = (a_h*(a_w^3))/12.0;
      GA = a_G*a_area;
      EI = a_E*a_inertia;
      EA = a_E*a_area;
      a_L = 20.0e-6;
...
%    end   
    [xx,yy,pp]=results(x,column_length,nstep,nu,test,ndm,num_basis_functions, num_output_points,output_interval);   
...
function [xx,yy,pp]=results(x,column_length,nstep,nu,test,ndm,num_basis_functions,  num_output_points,output_interval)  
...
xx=disp(1)/column_length; yy=disp(2)/column_length; pp=disp(3)/(pi/2);
