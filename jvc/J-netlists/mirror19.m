...
%micromirror
%net=cho_load('mirror19.m');figure(1);cho_display(net);
%net=cho_load('mirror19.m');[q,k]=cho_dc(net);figure(1);cho_display(net,q);
%clear all;tic;net=cho_load('mirror19.m');figure(1);cho_display(net);toc
%clear all;tic;net=cho_load('mirror19.m');[q,k]=cho_dc(net);figure(1);cho_display(net,q);toc

f3d * [bridgecenter][F=2600u oz=-pi/2]
...
[
   ...
   %rim
   semicircularbeam layer1 [plate_bottom plate_right]  [radius=radius_rim w=w_rim h=h_rim alpha=arcplate oz=0*oz_plate oy1=pi/2 oz1=(-pi/2+theta) L1=L oy2=theta oz2=pi L2=L] 
   semicircularbeam layer1 [plate_right  plate_top]    [radius=radius_rim w=w_rim h=h_rim alpha=arcplate oz=1*oz_plate oy1=pi/2 oz1=(-pi/2+theta) L1=L oy2=theta oz2=pi L2=L]
   semicircularbeam layer1 [plate_top    plate_left]   [radius=radius_rim w=w_rim h=h_rim alpha=arcplate oz=2*oz_plate oy1=pi/2 oz1=(-pi/2+theta) L1=L oy2=theta oz2=pi L2=L]
   semicircularbeam layer1 [plate_left   plate_bottom] [radius=radius_rim w=w_rim h=h_rim alpha=arcplate oz=3*oz_plate oy1=pi/2 oz1=(-pi/2+theta) L1=L oy2=theta oz2=pi L2=L]
]
