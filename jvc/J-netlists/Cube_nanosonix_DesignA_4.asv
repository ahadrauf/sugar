% p.V=0; net=cho_load('Cube_nanosonix_DesignA_4.m',p); q=cho_dc(net);
% figure(1); clf; cho_display(net,q); one = q(lookup_coord(net, 'c7', 'y'))
...
beam3dlink p0 [g1 b2] [l=L1/2+Lp1+(A2+2*p0)/2+tw/2+p0+corner w=tw L1=-corner]
