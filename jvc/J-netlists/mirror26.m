%micromirror
%net=cho_load('mirror26.m');figure(1);cho_display(net);
%net=cho_load('mirror26.m');[q,k]=cho_dc(net);figure(1);cho_display(net,q);
%clear all;tic;net=cho_load('mirror26.m');figure(1);cho_display(net);toc
%clear all;tic;net=cho_load('mirror26.m');[q,k]=cho_dc(net);figure(1);cho_display(net,q);toc

% clear all;tic;net=cho_load('mirror26.m');[q,k]=cho_dc(net);figure(1);cho_display(net,q);toc,theta=q(lookup_coord(net,'plate_top','rx')),cosine=q(lookup_coord(net,'bridgecenter','y'))

%verification:
%Mirror tilts about 0.14 rad for a cosine-beam displacement of 25um=2.5e-5m.

%tic;net=cho_load('mirror26.m');toc,[f,egv,dq]=cho_mode(net);toc,mode=1;cho_modeshape(net, f, egv, dq, 5,mode);toc
