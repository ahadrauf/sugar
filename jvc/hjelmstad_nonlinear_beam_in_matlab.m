%hjelmstad_nonlinear_beam_in_matlab
...
%c.... Execute numerical integration of Hessian and residual components
        ...
        for mpoint=1:npoints   %do mpoint = 1,npoints
          [mpoint,npoints,dz,xlength,factor]=simpson(mpoint,npoints,dz,xlength,factor); %call simpson(mpoint,npoints,dz,xlength,factor)
          [A,b,D,x,z,xlength,factor,ndm,nbasis]=fcn(A,b,D,x,z,xlength,factor,ndm,nbasis);    %call fcn(A,b,D,x,z,xlength,factor,ndm,nbasis)
          z = z + dz; %z = z + dz
        end %end do

%C.... Add end load terms to the residual and coefficient matrix
        for i=1:nbasis   %do i=1,nbasis
          [i,one,xlength,h,dh,nbasis]=basis(i,one,xlength,h,dh,nbasis);    %call basis(i,one,xlength,h,dh,nbasis)
          mm = 3*(i-1);
...
%c.... Test for convergence, if successful output values
        ...
        [x,xlength,nstep,nu,test,ndm,nbasis]=results(x,xlength,nstep,nu,test,ndm,nbasis);   %call results(x,xlength,nstep,nu,test,ndm,nbasis)
...
function [A,b,D,x,z,xlength,factor,ndm,nbasis]=fcn(A,b,D,x,z,xlength,factor,ndm,nbasis)   %subroutine fcn(A,b,D,x,z,xlength,factor,ndm,nbasis)
...
%c.... Compute displacements, derivatives, and current load factor clf 
      ...
      theta  = zero;
      for i=1:nbasis %do i=1,nbasis
         [i,z,xlength,h,dh,nbasis]=basis(i,z,xlength,h,dh,nbasis);    %call basis(i,z,xlength,h,dh,nbasis) 
         du = du + x(3*i-2)*dh;
         dw = dw + x(3*i-1)*dh;
...
%c.... Form stiffness matrix K and store it in matrix A
      for i=1:nbasis %do i=1,nbasis
        [i,z,xlength,hi,dhi,nbasis]=basis(i,z,xlength,hi,dhi,nbasis);   %call basis(i,z,xlength,hi,dhi,nbasis)
        for j=1:nbasis   %do j=1,nbasis
          [j,z,xlength,hj,dhj,nbasis]=basis(j,z,xlength,hj,dhj,nbasis); %call basis(j,z,xlength,hj,dhj,nbasis)
...
%c.... Form integral part of residual force and assemble into matrix b
        mm = 3*(i-1);
        [z,dm,dq,dp,x(ndm),1]=applied(z,dm,dq,dp,x(ndm),1);  %call applied(z,dm,dq,dp,x(ndm),1)
...
%c.... Form the integral part of load factor part of  matrix A
        [z,dm,dq,dp,x(ndm),2]=applied(z,dm,dq,dp,x(ndm),2)    %call applied(z,dm,dq,dp,x(ndm),2)
...
function [i,z,xlength,h,dh,nbasis]=basis(i,z,xlength,h,dh,nbasis)    %subroutine basis(i,z,xlength,h,dh,nbasis) 		            
...
function [i,z,xlength,h,dh,nbasis]=basis2(i,z,xlength,h,dh,nbasis)    %subroutine basis2(i,z,xlength,h,dh,nbasis)		             
...
function [z,dm,dq,dp,clf,n]=applied(z,dm,dq,dp,clf,n,  dmo,dqo,dpo)  %subroutine applied(z,dm,dq,dp,clf,n)                            
...
function [x,xlength,nstep,nu,test,ndm,nbasis,  npts,nnstep]=results(x,xlength,nstep,nu,test,ndm,nbasis,  npts,nnstep)  %subroutine results(x,xlength,nstep,nu,test,ndm,nbasis)
...
%c.... Write coefficients for current step
      if (plot)      %if (plot) then
        fprintf('load step=nstep=%g \t load factor=x(ndm)=%g \t iteration=nu=%g \t norm of residual=test=%g ',nstep, x(ndm), nu, test);  %write(4,2001) nstep, x(ndm), nu, test
        for i=1:nbasis   %do i=1,nbasis
          ii = 3*(i-1);
...
%c.... Compute and print current geometry of beam
      ...
      for ii=1:nplot+1   %do ii=1,nplot+1
        ...
        if (plot) fprintf('z*xlength=%g \t z*xlength+disp(1)=%g \t disp(2)=%g',z*xlength,z*xlength+disp(1),disp(2));    %write(3,3000) z*xlength,z*xlength+disp(1),disp(2)
        z = z + dz;
      end %end do
...
function [m,npoints,dz,xlength,factor]=simpson(m,npoints,dz,xlength,factor)   %subroutine simpson(m,npoints,dz,xlength,factor)
...
%      implicit double precision (a-h,o-z)
      ...
      elseif (n==0) %else if (n.eq.0) then
         factor = 4*c;   %factor = 4.d0*c
      else 
         factor = 2*c    %factor = 2.d0*c
      end %endif
%      return
%      end
