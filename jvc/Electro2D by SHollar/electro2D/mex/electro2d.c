...
/* static array S0_ (1 x 4) int, line 60 */
static int S0r_[] =
{
	0, 0, 0, 0, 
};
static mxArray S0_ = mccCINIT( mccINT, 1, 4, S0r_, 0 );
/* static array S1_ (1 x 4) int, line 66 */
static int S1r_[] =
{
	0, 0, 0, 0, 
...
/* static array S2_ (1 x 19) text, line 80: 'matrix calculations' */
...
/* static array S3_ (1 x 24) text, line 121: 'calculate force matrices' */
...
/* static array S7_ (1 x 26) text, line 179: 'calculate potential matri...' */
...
/* static array S8_ (1 x 23) text, line 227: 'invert potential matrix' */
 ...
 * M-File: C:/WINDOWS/Desktop/U C BERKELEY/CAD MEMS/Electro2D by SHollar/electro2D/mex/electro2d.m
 *
 ...
 *       atan        	<function>
 *       capacitance 	real vector/matrix
...
)
{
   ...
   mcmSetLineNumber(0);
   {
      ...
      {
         mexErrMsgTxt( "variable permitivity undefined, line 52" );
      }
      ...
      {
         mexErrMsgTxt( "variable z_depth undefined, line 54" );
      }
      ...
      if(mccNOTSET(&seg1))
      {
         mexErrMsgTxt( "variable seg1 undefined, line 61" );
      }
      ...
      for (I0_ = 0; I0_ < mccN(&IM0_); ++I0_)
      {
         ...
         {
            mexErrMsgTxt( "variable delta_approx undefined, line 62" );
         }
         Mprhs_[0] = &RM0_;
         Mprhs_[1] = &delta_approx;
         Mplhs_[0] = &RM1_;
         mccCallMATLAB(1, Mplhs_, 2, Mprhs_, "dissect", 62);
      ...
      if(mccNOTSET(&seg2))
      {
         mexErrMsgTxt( "variable seg2 undefined, line 67" );
      }
      ...
      for (I1_ = 1; I1_ <= I0_; I1_ = I1_ + 1)
      {
         ...
         if(mccNOTSET(&delta_approx))
         {
            mexErrMsgTxt( "variable delta_approx undefined, line 68" );
         }
         Mprhs_[0] = &RM1_;
         Mprhs_[1] = &delta_approx;
         Mplhs_[0] = &RM0_;
         mccCallMATLAB(1, Mplhs_, 2, Mprhs_, "dissect", 68);
      ...
      if(mccNOTSET(&delta_approx))
      {
         mexErrMsgTxt( "variable delta_approx undefined, line 123" );
      }
      {
         int i_, j_;
         int m_=1, n_=1, cx_ = 0;
      ...
      /* sub_force_matrices; */
      Mplhs_[0] = 0;
      mccCallMATLAB(0, Mplhs_, 0, Mprhs_, "sub_force_matrices", 130);
      /* sub_x_sum=sub_x; */
      ...
      /* sub_force_matrices; */
      Mplhs_[0] = 0;
      mccCallMATLAB(0, Mplhs_, 0, Mprhs_, "sub_force_matrices", 140);
      ...
      mccCatenateRows(&volt_m, &volt_m1, &volt_m2);
      ...
      Mplhs_[0] = &RM8_;
      mccCallMATLAB(1, Mplhs_, 1, Mprhs_, "inv", 232);
      ...
      /* charge_1=tot_charge(1:size_volt1); */
      {
         ...
         double *p_tot_charge;
         int I_tot_charge=1;
         m_ = mccCalcSubscriptDimensions(m_, &n_, 1, ((int)(size_volt1 - 1) + 1), &tot_charge);
         mccAllocateMatrix(&charge_1, m_, n_);
         I_charge_1 = (mccM(&charge_1) != 1 || mccN(&charge_1) != 1);
         p_charge_1 = mccPR(&charge_1);
         I_tot_charge = (mccM(&tot_charge) != 1 || mccN(&tot_charge) != 1);
         p_tot_charge = mccPR(&tot_charge) + ((int)(1 - .5));
         if (m_ != 0)
         {
            for (j_=0; j_<n_; ++j_)
            {
               for (i_=0; i_<m_; ++i_, p_charge_1+=I_charge_1, p_tot_charge+=I_tot_charge)
               {
                  *p_charge_1 = *p_tot_charge;
               }
            }
         }
      }
      ...
      I0_ = mccGetDimensionSize(&tot_charge, 1);
      mccIntColon2(&IM1_, (size_volt1+1), I0_);
      {
         int i_, j_;
         int m_=1, n_=1, cx_ = 0;
         double *p_charge_2;
         ...
         int *p_IM1_;
         int I_IM1_=1;
         m_ = mccCalcSubscriptDimensions(m_, &n_, mccM(&IM1_), mccN(&IM1_), &tot_charge);
         ...
         if (m_ != 0)
         {
            for (j_=0; j_<n_; ++j_)
            {
               for (i_=0; i_<m_; ++i_, p_charge_2+=I_charge_2, p_IM1_+=I_IM1_)
               {
                  *p_charge_2 = mccPR(&tot_charge)[((int)(*p_IM1_ - .5))];
               }
            }
         }
      }
      ...
      mccConjTrans(&RM8_, &charge_2);
      mccConjTrans(&RM7_, &F_xx);
      mccRealMatrixMultiply(&RM6_, &RM8_, &RM7_);
      mccRealMatrixMultiply(&FF_X, &RM6_, &charge_1);
      /* FF_Y=charge_2'*F_yy'*charge_1;	 */
      mccConjTrans(&RM6_, &charge_2);
      mccConjTrans(&RM7_, &F_yy);
      mccRealMatrixMultiply(&RM8_, &RM6_, &RM7_);
      mccRealMatrixMultiply(&FF_Y, &RM8_, &charge_1);
      /* mag_F=sqrt(FF_X^2+FF_Y^2); */
      mccPower(&RM8_, &FF_X, mccTempMatrix(2, 0., mccINT, 0 ));
      mccPower(&RM7_, &FF_Y, mccTempMatrix(2, 0., mccINT, 0 ));
      {
         int i_, j_;
         int m_=1, n_=1, cx_ = 0;
         double *p_RM6_;
         ...
         if (m_ != 0)
         {
            for (j_=0; j_<n_; ++j_)
            {
               for (i_=0; i_<m_; ++i_, p_RM6_+=I_RM6_, p_RM8_+=I_RM8_, p_RM7_+=I_RM7_)
               {
                  *p_RM6_ = (*p_RM8_ + *p_RM7_);
               }
            }
         }
      }
      ...
      /* capacitance=1e-12*z_axis*delta_approx... */
      {
         ...
         double *p_z_axis;
         int I_z_axis=1;
         m_ = mcmCalcResultSize(m_, &n_, mccM(&z_axis), mccN(&z_axis));
         mccAllocateMatrix(&RM6_, m_, n_);
         I_RM6_ = (mccM(&RM6_) != 1 || mccN(&RM6_) != 1);
         p_RM6_ = mccPR(&RM6_);
         I_z_axis = (mccM(&z_axis) != 1 || mccN(&z_axis) != 1);
         p_z_axis = mccPR(&z_axis);
         if (m_ != 0)
         {
            for (j_=0; j_<n_; ++j_)
            {
               for (i_=0; i_<m_; ++i_, p_RM6_+=I_RM6_, p_z_axis+=I_z_axis)
               {
                  *p_RM6_ = (1e-12 * (double) *p_z_axis);
               }
            }
         }
      }
      mccRealMatrixMultiply(&RM7_, &RM6_, &delta_approx);
      mccSum(&RM8_, &charge_2);
      R0_ = (((mccPR(&seg1)[(5-1)*mccM(&seg1) + (1-1)]) - (mccPR(&seg2)[(5-1)*mccM(&seg2) + (1-1)])) + mcmEps());
      {
         int i_, j_;
         int m_=1, n_=1, cx_ = 0;
         double *p_RM5_;
         ...
         if (m_ != 0)
         {
            for (j_=0; j_<n_; ++j_)
            {
               for (i_=0; i_<m_; ++i_, p_RM5_+=I_RM5_, p_RM8_+=I_RM8_)
               {
                  *p_RM5_ = (*p_RM8_ / (double) R0_);
               }
            }
         }
      }
      mccAbs(&RM0_, &RM5_);
      mccRealMatrixMultiply(&capacitance, &RM7_, &RM0_);
      ...
      mccReturnFirstValue(&plhs_[0], &capacitance);
      mccReturnValue(&plhs_[1], &FF_X);
      mccReturnValue(&plhs_[2], &FF_Y);
   }
   return;
}
