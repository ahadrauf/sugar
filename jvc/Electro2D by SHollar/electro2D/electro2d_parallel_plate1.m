...
%figure(1);plot([lhs max(lhs)+1], -Q{1}([lhs max(lhs)+1])+Q{1}(640)  );grid on
%sum( -Q{1}([lhs max(lhs)+1])+Q{1}(640) )
%sum( -Q{1}([lhs max(lhs)+1]) )
%figure(1);plot([lhs max(lhs)+1], -Q{1}([lhs max(lhs)+1]));grid on;

function [phi,gap,F_X,F_Y,cap,totQ,F_pp,capacitance_pp,Qpp,Q,rhs,lhs,bhs,ths,Qppsc,dQppsc,deltaQ]=electro2d_parallel_plate1
%init
D = [];
